...
- **Data transfer**:
  Props are also used to fill or transfer data to components that comes from resources, like `createResource` calls.
  Resulting in a component that reacts to changes in data in real-time.
  ...
  When order influence the element's behavior, users must define the expressions in the order that the element expect.
