...
This allows each each effect to independently track its own dependencies, without affect the effect that it is nested within.
...
In situations where you just want to run a side effect **once**, you can use the [`onMount`](/reference/onMount) function.
